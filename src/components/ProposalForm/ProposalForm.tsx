...
export default function ProposalForm(
  ...
  // const thread = proposalState.thread
  const blocks = proposalState.blocks
  const activeBlock = blocks[proposalState.activeBlockIndex]
  const nextBlock: ProposalBlockType | undefined =
    blocks[proposalState.activeBlockIndex + 1]
  ...
  const onInitVideo = useNonReactiveCallback(() => {
    if (initVideoUri) {
      selectVideo(activeBlock.id, initVideoUri)
    }
  })
  ...
  }, [proposalState, closeAllDialogs, discardPromptControl, onClose])
    ...
    blocks.every(
      block =>
        ...
        !(
          ...
          block.embed.media.video.status === 'error'
        ),
    )
    ...
    try {
      ...
      if (postUri) {
        ...
        for (let post of blocks) {
          logEvent('post:create', {
            ...
            isPartOfThread: blocks.length > 1,
      ...
      if (blocks.length > 1) {
        logEvent('thread:create', {
          postCount: blocks.length,
          isReply: false, // !!replyTo,
        })
      }
    }
  ...
  }, [blocks, onPressPublish, publishOnUpload])
  ...
  useEffect(() => {
    if (proposalState.mutableNeedsFocusActive) {
      proposalState.mutableNeedsFocusActive = false
  ...
  const isLastThreadedPost = blocks.length > 1 && nextBlock === undefined
  const {
    scrollHandler,
  ...
  const footer = (
    <>
      ...
      <Footer
        ...
        //   !isEmptyBlock(activeBlock) && (!nextBlock || !isEmptyBlock(nextBlock))
        // }
        onError={setError}
        // onEmojiButtonPress={onEmojiButtonPress}
        onSelectVideo={selectVideo}
        onAddPost={(blockType: ProposalBlockType['type']) => {
          proposalDispatch({
            type: 'add_block',
            blockType,
          })
  ...
  // const isWebFooterSticky = !isNative && blocks.length > 1
  return (
    <BottomSheetPortalProvider>
      <KeyboardAvoidingView
        testID="proposalPostView"
            ...
            {blocks.map((block, index) => (
              <React.Fragment key={block.id}>
                <ProposalBlock
                  ...
                  textInput={block.id === activeBlock.id ? textInput : null}
                  // isFirstPost={index === 0}
                  // isPartOfThread={thread.posts.length > 1}
                  // isReply={index > 0 || !!replyTo}
                  isActive={block.id === activeBlock.id}
                  canRemoveBlock={block.type !== 'title' && blocks.length > 2}
                  ...
                  onError={setError}
                />
                {/* {isWebFooterSticky && block.id === activeBlock.id && (
...
let ProposalBlock = React.memo(function ProposalBlock({
  post,
  dispatch,
  textInput,
  isActive,
  ...
  const selectTextInputPlaceholder =
    post.type === 'title'
      ? '请输入提案名称'
      : post.type === 'module-title'
      ? '请输入模块标题'
      : '请输入提案内容' //_(msg`What's up?`)
  const discardPromptControl = Prompt.usePromptControl()
  ...
  const isProposalTitle = post.type === 'title'
  const isModuleTitle = post.type === 'module-title'
  return (
    <View
      style={[
        a.mx_lg,
...
function Footer({
  ...
  onError: (error: string) => void
  onSelectVideo: (postId: string, asset: ImagePickerAsset) => void
  onAddPost: (blockType: ProposalBlockType['type']) => void
}) {
  const t = useTheme()
  const {_} = useLingui()
  ...
  if (post.type === 'title') return null
...
export function useProposalFormRef() {
...
function isEmptyBlock(post: ProposalBlockType) {
  return (
